...
**Router tree:** `apps/server/src/routers/index.mts` composes feature routers (`auth`, `space`, `account`, `event`, `envelop`, `expenseCategory`, `transaction`, `allocation`, `analytics`, `file`, `user`, `personal`, `pin`, `reckoning`, `health`). Each feature router in `routers/<name>.mts` imports procedures from `procedures/<name>/<action>.mts` — this one-procedure-per-file convention is the norm; follow it for new endpoints. The `personal.*` router powers the virtual "My money" space at `/s/me` — every analytics procedure has a cross-space personal twin; see project spec §6.5 and engineering spec §5.6. The `pin.*` router (migrations 043+044) stores transaction-entry defaults — Account is per-user-per-space, Envelope/Event are space-wide; see `contexts/modules/server/pin.md`.
