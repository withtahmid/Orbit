...
## Files
- Main page: `apps/web/src/pages/space/OverviewPage.tsx` (defines every card inline plus the `OV_STYLES` block, the `TodayBand`, `PersonalSpaceBand`, an over-allocation banner, and many derived chart components).
...
- `analytics.envelopeUtilization` + `personal.envelopeUtilization` — month-period envelopes for top-categories and the allocation donut (`:96-104`).
- `analytics.priorityBreakdown` — priority donut, real-space only (`:106-109`).
- `event.listBySpace` — upcoming events list, real-space only (`:111-114`).
...
- The over-allocation banner renders (real-space only) when more money is allocated to envelopes than the space holds; it reads `summary.unallocated` and prompts the user to deallocate or record income (`OverviewPage.tsx:496-516`). There is no borrow or reckoning banner.
