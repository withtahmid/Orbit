...
- Per-view detail: `ROUTES.spaceAnalyticsDetail(id, view)` -> `/s/:spaceId/analytics/:view` (`apps/web/src/router/routes.ts:27`). Note the router uses one explicit child route per view name (`apps/web/src/router/index.tsx:207-250`), so the dynamic `:view` slug is informational — only the eleven enumerated paths actually resolve.
- Lazy-imported in `apps/web/src/router/index.tsx:39-72`; mounted at `apps/web/src/router/index.tsx:203-250` under `SpaceLayout`.
- Guards: `ProtectedRoute` -> `CurrentSpaceProvider` -> `SpaceLayout`. All detail views are personal-aware (each query has a personal twin guarded with `{ enabled: isPersonal }`); the views that don't have a personal twin (allocations, matrix, priority) just call the real-space proc and rely on the SpaceLayout sidebar hiding analytics in personal mode — except `analytics` itself is in the personal nav, so those three views WILL break on `/s/me`.

## Files
- Index: `apps/web/src/pages/space/analytics/AnalyticsPage.tsx` — a static `ENTRIES` array (`:35-122`) renders 11 tiles. No tRPC calls of its own. Each tile is a `<Link>` to `ROUTES.spaceAnalyticsDetail`.
...
- `AllocationsView.tsx` — money-partitioning view (accounts -> envelopes/plans). Procs: `analytics.allocations` (`:73`), `plan.allocationListBySpace` (`:227,523`), `analytics.planProgress` (`:228`). Real-space only — no personal twin.
- `MatrixView.tsx` — envelope-by-account heatmap. Procs: `analytics.allocations` (`:45`). Real-space only.
...
- The dynamic `:view` slug in `ROUTES.spaceAnalyticsDetail` is convention only — the actual router declares 11 explicit child routes (`apps/web/src/router/index.tsx:208-250`). Adding a new view requires both a new tile in `ENTRIES` AND a new route entry.
- `AllocationsView`, `MatrixView`, and `PriorityView` lack personal twins (the real-space proc has no `personal.X` equivalent). They render in `/s/me` because Analytics is in the personal nav, but they will fail since the proc rejects the synthetic `id: "me"` space id.
