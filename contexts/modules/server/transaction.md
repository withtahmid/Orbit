...
- Available-balance is checked client-of-trigger: we read `account_balances.balance` (`utils/resolveAvailableBalance.mts:14`) and reject before insert. After the trigger fires, the balance reflects the new row.
- Fees count as expense everywhere downstream (`030:18-19`). The `topCategories`, `categoryBreakdown`, `envelopeUtilization`, `cashFlow`, `transactionFilteredTotals`, and reckoning SQL all `UNION ALL` a row of `(fee_expense_category_id, fee_amount)` into the expense population — see the analytics module doc for the canonical pattern.
...
- Strict-mode gate (`resolveStrictGate`) fires only on expense / transfer / adjust — **not** on income. Income is always allowed because adding cash never deepens an overspend.
...
- Strict-mode gate: `procedures/space/utils/resolveStrictGate.mts` (see reckoning module).
