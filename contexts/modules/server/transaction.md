...
- Signed effect matches the balance trigger (migration `018`, folded with fees at `030`): +amount to destination for income/transfer/adjustment, -amount from source for expense/transfer/adjustment. Transfer fees are separate `expense` rows and fall out naturally.
