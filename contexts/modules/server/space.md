...
- Composes procedures:
  ...
  - `space.update` — rename a space and/or change `budgetMode`.
...
- **`updateSpace`** (`procedures/space/update.mts:8`) — Auth: role-gated. Input: `{ spaceId, name?, budgetMode? }` where `budgetMode ∈ ('flexible'|'strict')`. Output: `{ id, name, budgetMode }`. **Owner-only if `budgetMode` is provided**; otherwise `owner` or `editor`. Sets `updated_by = ctx.auth.user.id`. Empty input still issues an UPDATE (no early-out).
...
- **`listSpaces`** (`procedures/space/list.mts:6`) — Auth: authorized. No input. Returns spaces the caller is a member of, projected to `{ id, name, budgetMode, myRole }`. Uses `z.enum(...).parse(...)` to cast Kysely's codegen `ArrayType` union to the literal at the boundary.
...
## Database tables
- **`spaces`** (`migrations/0004_create_spaces_table.mts`, extended by `037_space_budget_mode.mts`). Columns: `id uuid PK`, `name varchar(255) NOT NULL`, `created_at`, `updated_at`, `created_by uuid → users.id` (NOT NULL), `updated_by uuid → users.id` (NOT NULL), `budget_mode text NOT NULL DEFAULT 'flexible'` with `CHECK budget_mode IN ('flexible','strict')`. After migration `027_fk_on_delete_restrict_for_created_by.mts` the `created_by`/`updated_by` FKs use `ON DELETE RESTRICT` so a user can't be deleted while they own creator history.
...
- `resolveStrictGate` (`procedures/space/utils/resolveStrictGate.mts:19`) is a helper used by transaction-creating procedures elsewhere — it checks `spaces.budget_mode` and blocks new transactions if the caller has unresolved past-month overspends in strict mode. It belongs conceptually to space governance but is invoked from outside this module.
- Cascading deletes from `spaces.id` reach `space_members`, `space_accounts`, `envelops` → `expense_categories` / `envelop_allocations`, `events`, `transactions`, `plans`, `reckoning_acknowledgments`, etc. `deleteSpace` does not need to manually clean these up.
...
- `apps/server/src/procedures/space/utils/resolveStrictGate.mts` — strict-mode budget gate consumed by transaction procedures.
- `contexts/modules/server/auth.md` — `findUserByEmail` is the typical front-end lookup before calling `addMembers`.
- `apps/server/src/db/kysely/migrations/037_space_budget_mode.mts` — defines the `budget_mode` column and check constraint.
