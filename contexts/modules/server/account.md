...
- **`deleteAccount`** (`procedures/account/delete.mts:8`) — Auth: account `owner`. Deletes the `accounts` row inside a transaction; FKs cascade to balance/membership/sharing rows and will fail (FK RESTRICT) if rows that pin the account (e.g. `transactions`) still reference it. (Allocations no longer pin to an account — migration `048` dropped `envelop_allocations.account_id`, and `plan_allocations` was removed with plans in `046` — so allocations are no longer a delete guard.)
...
- **`unshareAccountFromSpace`** (`procedures/account/unshareFromSpace.mts:17`) — Auth: account `owner` OR space `owner`. Refuses if it would leave the account in zero spaces (`BAD_REQUEST` "delete the account instead"). Refuses if the space still has any `transactions` referencing the account (`procedures/account/unshareFromSpace.mts:87-106`). (Allocations are space-wide now and no longer pin to an account, so they're no longer an unshare guard.)
...
- `unshareFromSpace` will refuse on dangling `transactions` — this is the canonical pattern for "soft FK across the space boundary". (Allocations used to participate in this guard via `envelop_allocations.account_id`, but migration `048` dropped that column — allocations are space-wide now.)
...
- `contexts/modules/server/envelop.md` — envelope allocations (now space-wide; no longer pinned to an account).
