...
**Last rewritten:** after the cadence + per-account allocation work and the
cross-space account sharing feature.
...
- [`apps/server`](../apps/server) — Node.js + Express + **tRPC v11**
  ...
  applied manually via `pnpm --filter backend migrate`.
...
3001, the `orbit` service running `pnpm dev`).
...
- **`users`** — `id`, `email (unique)`, `password_hash`, `first_name`,
  `last_name`, `avatar_url`, `created_at`.
...
### 11.4 Filtering ([`transaction.list`](../apps/server/src/procedures/transaction/list.mts))
...
- `/spaces` — space picker
- `/settings/profile`, `/settings/security` — user settings
...
- **Light mode** — the palette has a `:root, .dark` combined block; a
  ...
  is hardcoded.
- **Timezone** — period windowing uses Postgres server time via
  `DATE_TRUNC('month', NOW())`. No per-space timezone. Users far from
  server time can see month boundaries shift slightly.
...
│   ├── space/              # create, update, delete, list, memberList, add/remove/changeRole
...
├── hooks/
├── lib/                    # dates, money, entityStyle, entityIcons, utils, permissions
